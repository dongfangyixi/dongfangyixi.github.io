...
Action is missing both — with an asterisk on the curves we'll come to: robotics does have curves, but they run along an unexpected axis, measured on proxies that can lie. And underneath sits a deeper problem, one no amount of fuel would fix. It needs stating precisely.
...
And all three maximize at the same coordinates: contact. What's absent from that accounting: any impossibility, any paradox, any missing genius. A signal-quality problem, a price list, and a measurement gap. That is the optimistic reading — paradoxes don't yield to engineering, but noise can be modeled, shortcuts can be regularized away, prices fall on published curves, and better evaluators are an engineering project already underway.
