...
![The causal chain of one timestep: hidden state z and tremor ε produce the action — the raw command — which the arm's inertia filter smooths into the next observation; the action head's target sits before the filter with measured floor 0.29, the vision head's target after it with floor at most 0.0025](/images/blog/action-bottleneck/fig-chain.svg)

*Where each head taps the chain. The action head's target sits upstream of the arm's filter: z and ε land in it at full strength, and three-tenths of its variance (B<sub>A</sub> ≈ 0.29, measured in Claim 2) is undetermined by the input. The vision head's target sits downstream: the same unknowns arrive scaled by the filter and buried under momentum, an undetermined share of at most a quarter of one percent (B<sub>V</sub> ≤ 0.0025, the innovation's variance share — Claim 3). Neither head can see z or ε — the chain decides how much that ignorance costs each of them.*
...
**The ablation that pins the mechanism.** If missing information is really the cause, handing the model the missing information should erase the effect — and it does. Give the action head the hidden state itself, the coin and the style parameter, as two extra input columns: the memorizer's validation error drops from 0.62 to 0.38, within a hair of the 0.36 that pure jitter alone produces in Claim 1's table — the entire decision-driven part of the error disappears<sup><a href="#ref-27">[27]</a></sup>. This also retires a rival reading of the curves above: that the action target is simply a *harder function* to fit — bimodal, discontinuous — than near-linear physics. Same policy, same eight episodes; only the input changed, and the unpredictability went with it. (One wrinkle worth keeping: reveal *z* to the *gradient-trained* network at this data size and overfitting gets catastrophically *worse* — training error dives to 0.12, while validation error climbs to 8.7, fourteen times the hidden-*z* network's 0.60. With only eight episodes, the style parameter doubles as an episode ID — the causal-confusion trap Claim 4 describes — and the network keys its memorized episodes on it, then extrapolates wildly on fresh values. Missing information is the disease, but the cure is not the input's mere presence; it is enough data to bind the new input to its consequences.)
...
**Test.** Half of the claim is testable on the rig: hold the frame budget fixed and trade frames for episodes. With ~170 training frames, the memorizer's action validation error falls from 0.85 at 8 episodes to 0.76 at 16 and 0.75 at 32 — while halving the frames of the same 8 episodes barely moves it, 0.85 to 0.88 (30 seeds)<sup><a href="#ref-27">[27]</a></sup>. Frames inside an episode are nearly redundant; the episode is the unit that carries information. (On a rig this small the vision head also gains from episode spread, since new episodes visit new states — so this pins the frame-redundancy half of the claim, not the diversity exponent.) The other half — that generalization scales with *conditions* — needs real scale, and the cleanest real-robot data-scaling study on record supplies it: 40,000+ demonstrations, 15,000+ evaluation rollouts; demonstrations *per environment* saturate at around fifty, while generalization follows a power law in the number of environments and objects<sup><a href="#ref-40">[40]</a></sup>. Counted the way that matters, a dataset's size is its number of conditions, not its number of frames.
...
| 4 | The action head can cheat, and cheats do not pay in the loop | Copying inertia or episode cues beats the open-loop metric without adding capability | Give one network the previous command; then let both drive | Open-loop error −31%; closed-loop 70% vs 73% |
| 5 | Action data holds fewer real samples than it appears | Only decision points carry new information; the rest is autocorrelated filler | Fixed frame budget on the rig (episodes beat frames); real-robot data-scaling study<sup><a href="#ref-40">[40]</a></sup> | At equal frames, 16 episodes beat 8 (0.76 vs 0.85); demos per environment saturate at ~50 |
...
— read: if the learned policy errs with probability *δ* per step on the expert's states, its shortfall over a task of *T* steps grows not like *δT* (errors adding up) but like *δT²* — because one error at step *t* can poison all remaining *T − t* steps, and there are ~*T* opportunities to make it<sup><a href="#ref-46">[46]</a></sup>. (The theorem is proved for discrete actions and 0–1 loss; treat the continuous-control version as directional.) (This is the imitation literature's classic *O(εH²)* compounding bound, with *H* the horizon — restated in *δ* and *T*, the trajectory length from the problem statement, because this essay has already spent *ε* on the demonstrator's tremor and *H*(·) on entropy.) That quadratic is provably a worst-case lower bound for any offline method, not an artifact of weak algorithms — the missing information is *recovery behavior*, which demonstration datasets don't contain<sup><a href="#ref-47">[47]</a></sup>.
