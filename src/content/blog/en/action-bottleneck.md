...
One choice in this setup needs explaining: training and validation come from the same dataset, with nothing changed between them. Why hold everything fixed? Because there is a rival explanation to rule out. Robots also fail when the scene changes — move the camera, change the lighting — and studies show much of *that* failure comes from the vision encoder, not from the action part, and can be repaired without touching the action part<sup><a href="#ref-27">[27]</a></sup>. In our puzzle nothing visual changes, and the action head misbehaves anyway. So the cause cannot be the camera or the vision encoder.
